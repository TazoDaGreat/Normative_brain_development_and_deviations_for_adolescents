...
library(janitor)
# ==================================== data ==========================================
cortical_thickness <- read_csv("/Users/jeffee/Library/CloudStorage/Box-Box/ABCD Tabulated Data/5.1/core//imaging/mri_y_smr_thk_dsk.csv") %>% 
  filter(eventname == "baseline_year_1_arm_1" | eventname == "2_year_follow_up_y_arm_1") # filter to baseline and year two

covariates <- read_csv("/Users/jeffee/Library/CloudStorage/Box-Box/2025-06-04_JC.csv") %>%
...
complete_data_plus_combat <- merge(x = complete_data, y = data_harmonized[,c(1,2,4:74)], by.x=c('src_subject_id', 'eventname'), by.y=c('src_subject_id', 'eventname'))
...
forest_plot <- all_the_mods %>%
  ...
  scale_color_manual(values = wes_palette("Darjeeling1")[2:4]) +
