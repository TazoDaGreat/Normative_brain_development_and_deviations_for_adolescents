...
thickness_centile_male_wider <- thickness_centile_male %>%
  select(src_subject_id, eventname, interview_age, centile, race_ethnicity) %>%
  pivot_wider(names_from = eventname, values_from = c(interview_age, centile))

thickness_centile_female_wider <- thickness_centile_female %>%
  select(src_subject_id, eventname, interview_age, centile) %>%
  pivot_wider(names_from = eventname, values_from = c(interview_age, centile))
```
