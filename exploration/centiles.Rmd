...
thickness_centile_male_wider %>%
  ...
  labs(title = "Individual Centile Scores by Race",
       subtitle = "Scores at Baseline",
       x = "Centile",
       y = "Individual Count")

thickness_centile_male_wider %>%
  drop_na() %>% 
  mutate(centile_bin = cut(centile_year_2, breaks = seq(0, 1, by = 0.25), include.lowest = TRUE)) %>%
  ggplot(aes(x = centile_bin, fill = race_ethnicity)) +
  geom_bar(position = "stack") +
  labs(title = "Individual Centile Scores by Race",
       subtitle = "Scores at Year 2",
       x = "Centile",
       y = "Individual Count")

thickness_centile_male_wider %>%
  drop_na() %>% 
  mutate(centile_bin = cut(centile_year_4, breaks = seq(0, 1, by = 0.25), include.lowest = TRUE)) %>%
  ggplot(aes(x = centile_bin, fill = race_ethnicity)) +
  geom_bar(position = "stack") +
  labs(title = "Individual Centile Scores by Race",
       subtitle = "Scores at Year 4",
       x = "Centile",
       y = "Individual Count")
