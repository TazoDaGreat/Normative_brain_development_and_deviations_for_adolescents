...
subtitle: "ABCD Study Team (Andrea Wiglesworth, Mark Fiecas, Kelly Duffy, Ellery Island)"
author: "Sarah Thomson and Maren Rusk"
date: "`r Sys.Date()`"
...
data_dir <- '/Users/marenrusk/Library/CloudStorage/Box-Box/ABCD Tabulated Data/5.1/core/' # UPDATE HERE FOR YOUR FILE PATH
#data_dir <-  '/Users/sarah/Library/CloudStorage/Box-Box/ABCD Tabulated Data/5.1/core/'

# Location of desired output directory of created csv file - if NULL, will output file into current working directory
#out_dir <- '/Users/sarah/Library/CloudStorage/Box-Box/ABCD-clean' # ['path',NULL]
out_dir <- '/Users/marenrusk/Library/CloudStorage/Box-Box/ABCD Data'
...
out_initials <- 'MSR' # ['initials_string']
...
# Acculturation Files
# ce_p_acc, ce_y_acc, ce_p_via, ce_y_via (only collected year 3 and on?)
phenx_p <- read.csv("culture-environment/ce_p_acc.csv") # parent language proficiency and preference
phenx_y <- read.csv("culture-environment/ce_y_acc.csv") # child language proficiency and preference
via_p <- read.csv("culture-environment/ce_p_via.csv") # parent vancouver acculturation index
via_y <- read.csv("culture-environment/ce_y_via.csv") # child vancouver acculturation index
```

```{r}
# Cleaning Acculturation
# mean scores only include requirement that all questions were answered, else (??)
via_p <- via_p |>
  select(src_subject_id, eventname, vancouver_q1_ddn_p, via_p_ss_hc, via_p_ss_hc_nt, via_p_ss_amer, via_p_ss_amer_nt)

via_y <- via_y |>
  select(src_subject_id, eventname, vancouver_q1_dd, via_ss_hc, via_ss_hc_nt, via_ss_amer, via_ss_amer_nt)
```
...
demog_bl <- demog_bl %>% 
  select(src_subject_id, demo_sex_v2, demo_race_a_p___10,
         ...
         demo_origin_v2, demo_prnt_origin_v2, demo_biofather_v2, demo_biomother_v2, demo_matgrandm_v2, demo_matgrandf_v2, demo_patgrandm_v2, demo_patgrandf_v2)
...
```{r}
files <- list(demog, demog_y, puberty, study_covars, mri, qc, scan_qtns, cbcl, bpm_y, ksad_new_df, phenx_p, phenx_y, via_p, via_y)
...
abcd_data <- abcd_data %>%
  # aggregate ksad questions to create meaningful, new variables 
    ...
    mutate(across(c(SI_ever_y, SI_ever_p,SI_ever_p_y, SA_ever_y,SA_ever_p,SA_ever_p_y, STB_highest_ever_y,STB_highest_current_y), ~as.factor(.x))) |>  # convert all new vars to factors  
       ...
       # Added: recoded logical binary version that checks for consistency across years using SI_ever_p_y
              mutate(si_ever = if_else(SI_ever_p_y == "never", 0, 1)) %>%
              # ensure SI is 1 if subject reported at any time point
              group_by(src_subject_id) %>%
              mutate(si_ever = cummax(si_ever)) %>%
              ungroup()
...
abcd_data <- abcd_data %>%
  ...
  # recoded logical binary depressive disorder ever variable that checks for consistency across years
  # using AnyDD_ever_p_y
  mutate(dd_ever_raw = case_when (
      AnyDD_ever_p_y == "never" ~ 0,
      is.na(AnyDD_ever_p_y) ~ NA_real_,
      TRUE ~ 1
    )) |>
  
  group_by(src_subject_id) |>
  mutate(
    # Create an indicator that becomes TRUE at first 1 and stays TRUE
    had_dd = cumany(dd_ever_raw == 1),
    
    # Recode: 1 if ever had DD *and* this or later, else keep original
    dd_ever = case_when(
      had_dd ~ 1,
      !had_dd & is.na(dd_ever_raw) ~ NA_real_,
      TRUE ~ dd_ever_raw
    )) |>
  #select(src_subject_id, AnyDD_ever_p_y, dd_ever, eventname)
  ungroup()
...
abcd_data <- abcd_data |>
  mutate(asian = ifelse(
           demo_race_a_p___14 == 1 |
           demo_race_a_p___15 == 1 |
           demo_race_a_p___16 == 1 |
           demo_race_a_p___17 == 1 |
  ...
  mutate(race_identity_count = rowSums(across(c(3, 4, 5, 6, 18, 186)))) |>
  mutate(asian_race_identity_count = rowSums(across(7:17))) |>
  mutate(multiracial = ifelse(race_identity_count > 1, 1, 0)) |>
  mutate(multiethnic = ifelse(asian_race_identity_count > 1, 1, 0)) |>
  mutate(monoethnic = ifelse(race_identity_count == 1 & asian_race_identity_count <= 1, 1, 0))
```


```{r}
abcd_long <- abcd_data |>
  pivot_longer(starts_with("demo_race_a_p"), names_to = "race", values_to = "race_response") %>% # pivot race/ethnicity variables
  filter(str_detect(race, "1[4-9]") | str_detect(race, "2[0-4]")) %>% # only interested in Asian categories 
  pivot_longer(c("multiethnic", "multiracial", "monoethnic"), names_to="ethnic_comp", values_to = "comp_response") %>%
   mutate(race = case_when (
    str_detect(race, "10") ~ "White",
    str_detect(race, "11") ~ "Black",
    str_detect(race, "12") ~ "Native American",
    str_detect(race, "13") ~ "Alaska Native",
    str_detect(race, "14") ~ "Native Hawaiian or Pacific Islander",
    str_detect(race, "15") ~ "Native Hawaiian or Pacific Islander",
    str_detect(race, "16") ~ "Native Hawaiian or Pacific Islander",
    str_detect(race, "17") ~ "Native Hawaiian or Pacific Islander",
    str_detect(race, "18") ~ "Asian Indian",
    str_detect(race, "19") ~ "Chinese",
    str_detect(race, "20") ~ "Filipino",
    str_detect(race, "21") ~ "Japanese",
    str_detect(race, "22") ~ "Korean",
    str_detect(race, "23") ~ "Vietnamese",
    str_detect(race, "24") ~ "Other Asian",
    str_detect(race, "25") ~ "Other Race",
    str_detect(race, "77") ~ "Refuse to answer",
    str_detect(race, "99") ~ "Don't Know",
  ))
```
