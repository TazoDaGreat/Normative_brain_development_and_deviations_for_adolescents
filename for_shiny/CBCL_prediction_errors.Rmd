...
region_spec <-brain_data_male_predicted %>% 
  filter(current_col_name == "smri_thick_cdk_banksstslh.combat") %>% 
  drop_na(cbcl_scr_dsm5_depress_r)

model <- glm(formula = cbcl_scr_dsm5_depress_r ~ centile_slope + centile_intercept,
             data = region_spec,
             family = "poisson")

region_spec$resid <- resid(model, type = "pearson")

region_spec_2 <-brain_data_male_predicted %>% 
  filter(current_col_name == "smri_thick_cdk_rrmdfrlh.combat") %>% 
  drop_na(cbcl_scr_dsm5_depress_r)

model_2 <- glm(formula = cbcl_scr_dsm5_depress_r ~ centile_slope + centile_intercept,
             data = region_spec,
             family = "poisson")

region_spec_2$resid <- resid(model_2, type = "pearson")

region_spec %>% 
  ggplot(aes(x = centile_slope, y = resid)) +
  geom_point()

region_spec %>%
  summarize(mean = mean(resid, na.rm = TRUE))
